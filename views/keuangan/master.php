...
<div id="keuangan-master-app" v-cloak class="container-fluid px-4 py-4">
    ...
    <!-- Navigation Tabs -->
    <ul class="nav nav-tabs border-bottom border-slate-200 mb-4" id="masterTabs" role="tablist">
    ...
    <div class="tab-content" id="masterTabsContent">
            ...
            <div class="row">
                    ...
                    <div class="card border-0 shadow-sm rounded-4 p-4 bg-white">
                        ...
                        <form @submit.prevent="saveKomponen">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control border-slate-200" v-model="formKomp.nama_komponen" placeholder="Contoh: SPP Bulanan, Uang Buku LKS, Kegiatan KTS" required>
                            </div>
                            <div class="mb-4">
                                <label class="form-label fw-semibold text-slate-700">Tipe Periode Pembayaran</label>
                                <select class="form-select border-slate-200" v-model="formKomp.tipe_periode">
                            ...
                            <div class="d-flex gap-2">
                                <button type="button" v-if="formKomp.id" @click="resetFormKomp" class="btn btn-light fw-bold flex-fill">Batal</button>
                                <button type="submit" class="btn btn-primary fw-bold flex-fill" :disabled="loadingKomp">Simpan</button>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <div class="col-12 col-md-8">
                    <div class="card border-0 shadow-sm rounded-4 overflow-hidden bg-white p-4">
                        <h5 class="fw-bold text-slate-800 mb-3">Daftar Komponen Biaya</h5>
                            ...
                            <table class="table table-hover align-middle">
                                ...
                                <tbody>
                                    <tr v-for="komp in komponenList" :key="komp.id">
                                        ...
                                        <td>
                                            <span class="badge rounded-pill px-3 py-2" :class="getPeriodeBadgeClass(komp.tipe_periode)">
                                                {{ komp.tipe_periode }}
                                            </span>
                                        </td>
                                        <td class="text-center">
                                            <button @click="editKomponen(komp)" class="btn btn-sm btn-outline-primary border-0 me-1" title="Edit">
                                                <i class="bi bi-pencil-square"></i>
                                            </button>
                                            <button @click="deleteKomponen(komp.id)" class="btn btn-sm btn-outline-danger border-0" title="Hapus">
        ...
        <div class="tab-pane fade" id="tarif-pane" role="tabpanel">
            <div class="row">
                ...
                <div class="col-12 col-md-4 mb-4">
                    <div class="card border-0 shadow-sm rounded-4 p-4 bg-white">
                        <h5 class="fw-bold text-slate-800 mb-3">Tautkan Tarif Baru</h5>
                        <form @submit.prevent="saveTarif">
                            <div class="mb-3">
                                ...
                                <select class="form-select border-slate-200" v-model="formTarif.komponen_id" required>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select border-slate-200" v-model="formTarif.tahun_ajaran_id" required>
                                    <option value="" disabled>-- Pilih Tahun Ajaran --</option>
                                    <option v-for="ta in listTa" :value="ta.id" :selected="ta.status === 'Aktif'">
                                        {{ ta.tahun_ajaran }} {{ ta.status === 'Aktif' ? '(Aktif)' : '' }}
                                    </option>
                                </select>
                            </div>
                            
                            <!-- Filter Kelas / Jenjang / Jalur Masuk (Pilih salah satu) -->
                            <div class="mb-3">
                                ...
                                <div class="form-check mb-1">
                                    ...
                                    <label class="form-check-label text-slate-700 fs-7" for="radioGeneral">Seluruh Siswa (General)</label>
                                </div>
                                <div class="form-check mb-1">
                                    ...
                                    <label class="form-check-label text-slate-700 fs-7" for="radioKelas">Spesifik per Kelas</label>
                                </div>
                                <div class="form-check mb-1">
                                    ...
                                    <label class="form-check-label text-slate-700 fs-7" for="radioJenjang">Spesifik per Jenjang</label>
                                </div>
                                <div class="form-check mb-3">
                                    ...
                                    <label class="form-check-label text-slate-700 fs-7" for="radioJalur">Spesifik per Jalur Masuk PPDB</label>
                                </div>
                            </div>

                            <!-- Dropdown dinamis target -->
                            <div class="mb-3" v-if="tarifTargetType === 'kelas'">
                                <label class="form-label fw-semibold text-slate-700">Pilih Kelas <span class="text-danger">*</span></label>
                                <select class="form-select border-slate-200" v-model="formTarif.kelas_id" required>
                                    <option value="" disabled>-- Pilih Kelas --</option>
                                    <option v-for="c in listKelas" :value="c.id">{{ c.nama_kelas }}</option>
                                </select>
                            </div>

                            <div class="mb-3" v-if="tarifTargetType === 'jenjang'">
                                <label class="form-label fw-semibold text-slate-700">Pilih Jenjang <span class="text-danger">*</span></label>
                                <select class="form-select border-slate-200" v-model="formTarif.jenjang_id" required>
                                    <option value="" disabled>-- Pilih Jenjang --</option>
                                    <option v-for="j in listJenjang" :value="j.id">{{ j.nama_jenjang }}</option>
                                </select>
                            </div>

                            <div class="mb-3" v-if="tarifTargetType === 'jalur'">
                                <label class="form-label fw-semibold text-slate-700">Jalur Masuk PPDB <span class="text-danger">*</span></label>
                                <input type="text" class="form-control border-slate-200" v-model="formTarif.jalur_masuk" placeholder="Contoh: Prestasi, KIP, Reguler" required>
                            </div>

                            <div class="mb-4">
                                <label class="form-label fw-semibold text-slate-700">Nominal Tarif (Rupiah) <span class="text-danger">*</span></label>
                                <div class="input-group">
                                    <span class="input-group-text bg-light border-slate-200 fw-bold">Rp</span>
                                    <input type="number" class="form-control border-slate-200" v-model="formTarif.nominal" placeholder="0" required>
                                </div>
                            </div>

                            <button type="submit" class="btn btn-primary fw-bold w-100" :disabled="loadingTarif">Simpan Tarif</button>
                        </form>
                    </div>
                </div>
                ...
                <div class="col-12 col-md-8">
                    <div class="card border-0 shadow-sm rounded-4 overflow-hidden bg-white p-4">
                        <h5 class="fw-bold text-slate-800 mb-3">Daftar Tarif Default</h5>
                        <div class="table-responsive">
                            <table class="table table-hover align-middle">
                                <thead class="table-light text-slate-700">
                                    <tr>
                                ...
                                <tbody>
                                    <tr v-for="t in tarifList" :key="t.id">
                                        ...
                                        <td>
                                            <span v-if="t.nama_kelas" class="badge bg-blue-100 text-blue-700 px-3 py-1">Kelas {{ t.nama_kelas }}</span>
                                            <span v-else-if="t.nama_jenjang" class="badge bg-purple-100 text-purple-700 px-3 py-1">Jenjang {{ t.nama_jenjang }}</span>
                                            <span v-else-if="t.jalur_masuk" class="badge bg-teal-100 text-teal-700 px-3 py-1">Jalur {{ t.jalur_masuk }}</span>
                                            <span v-else class="badge bg-slate-100 text-slate-700 px-3 py-1">Semua Siswa</span>
                                        </td>
                                        <td class="fw-semibold text-slate-700">Rp {{ formatNumber(t.nominal) }}</td>
                                        <td>{{ t.tahun_ajaran }}</td>
                                        <td class="text-center">
                                            <button @click="deleteTarif(t.id)" class="btn btn-sm btn-outline-danger border-0" title="Hapus">
                                                <i class="bi bi-trash"></i>
                                            </button>
                                        </td>
                                    </tr>
