    ...
    setup() {
        ...
        const fetchKelas = async () => {
            try {
                ...
                const response = await fetch('<?= $this->getBaseUrl() ?>/api/v1/keuangan/kelas' + query);
                const res = await response.json();
                if (res.success) {
        ...
        const searchSiswa = () => {
            ...
            searchTimeout = setTimeout(async () => {
                try {
                    ...
                    const jenjangParam = selectedJenjangId.value ? `&jenjang_id=${selectedJenjangId.value}` : '';
                    const response = await fetch(`<?= $this->getBaseUrl() ?>/api/v1/keuangan/cari-siswa?q=${encodeURIComponent(siswaSearch.value)}${tenantSuffix}${classParam}${jenjangParam}`);
                    const res = await response.json();
                    if (res.success) {
        ...
        const fetchTagihanSiswa = async (siswaId) => {
            try {
                const response = await fetch(`<?= $this->getBaseUrl() ?>/api/v1/keuangan/tagihan-siswa?siswa_id=${siswaId}`);
                const res = await response.json();
                if (res.success) {
        ...
        const checkoutPembayaran = async () => {
            ...
            try {
                const tenantSuffix = isSuperAdmin && selectedTenantId.value ? `?tenant_id=${selectedTenantId.value}` : '';
                const response = await fetch('<?= $this->getBaseUrl() ?>/api/v1/keuangan/bayar' + tenantSuffix, {
                    method: 'POST',
        ...
        const submitBatalPembayaran = async () => {
            ...
            try {
                const tenantSuffix = isSuperAdmin && selectedTenantId.value ? `?tenant_id=${selectedTenantId.value}` : '';
                const response = await fetch('<?= $this->getBaseUrl() ?>/api/v1/keuangan/batal-pembayaran' + tenantSuffix, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
