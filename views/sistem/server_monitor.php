        ...
        <div class="card border-0 shadow-sm rounded-4 p-4 mb-4" style="background:#fff;">
            ...
            <div class="bg-dark rounded-3 p-3 overflow-auto font-monospace text-light fs-8" style="height:350px;">
                <div v-if="!updateLog" class="text-secondary opacity-75">
                    ...
                    <code class="text-info">www-data ALL=(ALL) NOPASSWD: /var/www<?= $this->getBaseUrl() ?>/deploy.sh</code><br><br>
...
<script>
{
    ...
    const API_URL = '<?= $this->getBaseUrl() ?>/api/v1/super-admin/server-monitor/fetch';
    ...
    window.VueAppRegistry.register('#serverMonitorApp', {
        ...
        methods: {
            ...
            async triggerUpdate() {
                ...
                try {
                    const res = await axios.post('<?= $this->getBaseUrl() ?>/api/v1/super-admin/server-monitor/update-server');
            ...
            async submitNetworkConfig() {
                ...
                try {
                    const res = await axios.post('<?= $this->getBaseUrl() ?>/api/v1/super-admin/server-monitor/save-network', this.form);
                    if (res.data.success) {
                        Swal.fire({
                            icon: 'success',
                            title: 'Berhasil',
