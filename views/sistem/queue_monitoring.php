    ...
    window.VueAppRegistry.register('#queueMonitoringApp', {
        setup() {
            ...
            const fetchData = async (page = 1) => {
                ...
                try {
                    const response = await axios.get('<?= $this->getBaseUrl() ?>/api/v1/queue/data', {
                        params: {
                            page: page,
            ...
            const dispatchSimJob = async () => {
                ...
                try {
                    const response = await axios.post('<?= $this->getBaseUrl() ?>/api/v1/queue/dispatch', {
                        job_type: jobType,
                        payload: payload,
            ...
            const runWorkerOnce = async () => {
                ...
                try {
                    const response = await axios.post('<?= $this->getBaseUrl() ?>/api/v1/queue/run-worker');
            ...
            const retryJob = async (jobId) => {
                try {
                    const response = await axios.post('<?= $this->getBaseUrl() ?>/api/v1/queue/retry', { id: jobId });
                    if (response.data && response.data.success) {
                        Swal.fire({
                            icon: 'success',
            ...
            const deleteJob = (jobId) => {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        try {
                            const response = await axios.post('<?= $this->getBaseUrl() ?>/api/v1/queue/delete', { id: jobId });
                            if (response.data && response.data.success) {
                                Swal.fire({
                                    icon: 'success',
                                    title: 'Terhapus',
