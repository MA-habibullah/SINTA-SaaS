        ...
        <div class="tab-pane fade show active" id="pengumuman-content" role="tabpanel">
            <div class="glass-card rounded-3xl p-6 hover-lift mb-8">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 border-b border-slate-100 pb-4 gap-4">
                    ...
                    <form method="GET" action="<?= $this->getBaseUrl() ?>/informasi/pengumuman" class="d-flex flex-wrap gap-2 w-full md:w-auto">
                        ...
                        <?php if (!empty(array_filter($filters))): ?>
                            <a href="<?= $this->getBaseUrl() ?>/informasi/pengumuman" class="btn btn-light rounded-xl px-3 border border-slate-200 shadow-sm text-rose-500 hover:bg-rose-50 hover:border-rose-200 transition-colors py-2" title="Reset"><i class="bi bi-x-lg"></i></a>
                        <?php endif; ?>
                    </form>
                </div>
                                    ...
                                    <tr class="hover:bg-slate-50 transition-colors group">
                                        ...
                                        <td class="py-4">
                                            ...
                                            <div class="flex items-center gap-3">
                                                ...
                                                <?php if ($row['lampiran_file']): ?>
                                                    <a href="<?= $this->getBaseUrl() ?>/storage/app/public/<?= htmlspecialchars($row['lampiran_file']) ?>" target="_blank" class="text-xs bg-blue-50 text-blue-600 px-2 py-0.5 rounded border border-blue-100 font-semibold hover:bg-blue-100 transition-colors flex items-center gap-1 text-decoration-none">
                                        ...
                                        <td class="py-4 text-center">
                                            <div class="flex justify-center gap-2">
                                                ...
                                                <form action="<?= $this->getBaseUrl() ?>/informasi/pengumuman/delete" method="POST" class="m-0" onsubmit="return confirm('Hapus pengumuman ini secara permanen?');">
            ...
            <div class="glass-card rounded-3xl p-6 hover-lift mb-8">
                <div class="flex justify-between items-center mb-6 border-b border-slate-100 pb-4">
                    ...
                    <div class="flex items-center gap-3 w-full md:w-auto mt-3 md:mt-0">
                        <form method="GET" action="<?= $this->getBaseUrl() ?>/informasi/pengumuman" class="d-flex flex-wrap gap-2 m-0" id="filterKategoriForm">
                            <?php if ($isSuperAdmin): ?>
                                    ...
                                    <tr class="hover:bg-slate-50 transition-colors group">
                                        ...
                                        <td class="py-4 text-center">
                                            <div class="flex justify-center gap-2">
                                                ...
                                                <form action="<?= $this->getBaseUrl() ?>/informasi/pengumuman/kategori/delete" method="POST" class="m-0" onsubmit="return confirm('Menghapus kategori akan mengosongkan kategori pada pengumuman terkait. Lanjutkan?');">
...
<div class="modal fade" id="addKategoriModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-scrollable">
        <form action="<?= $this->getBaseUrl() ?>/informasi/pengumuman/kategori/store" method="POST" id="formKategori">
...
<div class="modal fade" id="addPengumumanModal" tabindex="-1">
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <form action="<?= $this->getBaseUrl() ?>/informasi/pengumuman/store" method="POST" enctype="multipart/form-data" id="formPengumuman">
            <div class="modal-content">
                <div class="modal-header">
...
window.loadEditorScript = function() {
    if (typeof ClassicEditor === 'undefined') {
        if (!document.querySelector('script[src*="ckeditor.js"]')) {
            ...
            script.src = '<?= $this->getBaseUrl() ?>/assets/js/ckeditor.js';
...
document.getElementById('addPengumumanModal').addEventListener('hidden.bs.modal', function () {
    ...
    document.getElementById('formPengumuman').action = '<?= $this->getBaseUrl() ?>/informasi/pengumuman/store';
...
document.getElementById('addKategoriModal').addEventListener('hidden.bs.modal', function () {
    ...
    document.getElementById('formKategori').action = '<?= $this->getBaseUrl() ?>/informasi/pengumuman/kategori/store';
...
window.editPengumuman = function(data) {
    ...
    document.getElementById('formPengumuman').action = '<?= $this->getBaseUrl() ?>/informasi/pengumuman/update';
...
window.editKategori = function(data) {
    ...
    document.getElementById('formKategori').action = '<?= $this->getBaseUrl() ?>/informasi/pengumuman/kategori/update';
