    ...
    window.VueAppRegistry.register('#schoolProfileApp', {
        setup() {
            ...
            const changeTenant = (event) => {
                ...
                window.location.href = `<?= $this->getBaseUrl() ?>/sekolah/identitas?tenant_id=${selectedId}`;
            };
            ...
            const submitProfile = async () => {
                ...
                try {
                    const response = await axios.post('<?= $this->getBaseUrl() ?>/api/v1/sekolah/update', formData, {
                        headers: {
