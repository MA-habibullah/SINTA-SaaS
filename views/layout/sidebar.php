    ...
    try {
        ...
        if (isset($_SESSION['logged_in']) && $_SESSION['logged_in'] === true) {
            if (($_SESSION['role_name'] ?? '') === 'super_admin') {
                $stmtUnread = $db->prepare("SELECT COUNT(*) FROM core.tickets WHERE admin_unread = TRUE AND is_active = TRUE");
                $stmtUnread->execute();
            } else {
                $stmtUnread = $db->prepare("SELECT COUNT(*) FROM core.tickets WHERE user_unread = TRUE AND is_active = TRUE AND tenant_id = ? AND user_id = ?");
