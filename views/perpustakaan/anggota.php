...
<div class="tab-content" id="anggotaTabsContent">
                            ...
                            <?php foreach ($data['anggota_list'] as $idx => $a): ?>
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <a href="/SINTA-SaaS/perpustakaan/cetak-label-thermal?barcode=<?= urlencode($a['no_anggota']) ?>" target="_blank" class="btn btn-outline-primary btn-sm rounded-2 me-1" title="Cetak Kartu Anggota">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-12 mt-4 text-center">
                            <div id="panelStatusLulus" class="alert alert-success d-none border-0 py-3 rounded-3">
                                ...
                                <a href="/SINTA-SaaS/perpustakaan/cetak-laporan-peminjaman" target="_blank" class="btn btn-success btn-sm rounded-3 px-4 fw-semibold"><i class="bi bi-printer me-1"></i> Cetak Surat Bebas Pustaka (PDF)</a>
                            </div>
    ...
    <div class="tab-pane fade" id="tamu-pane" role="tabpanel" aria-labelledby="tamu-tab">
        <div class="card border-0 shadow-sm rounded-4 p-4">
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <button type="button" class="btn btn-outline-success btn-sm rounded-3 px-3 fs-7" onclick="window.open('/SINTA-SaaS/perpustakaan/kios-pintu', '_blank')">
    ...
    <div class="tab-pane fade" id="pengaturan-pane" role="tabpanel" aria-labelledby="pengaturan-tab">
        <div class="card border-0 shadow-sm rounded-4 p-4">
            ...
            <form action="/SINTA-SaaS/api/v1/perpustakaan/pengaturan/simpan" method="POST" data-turbo="false">
...
function initAnggotaHandlers() {
    ...
    if (syncActionBtn) {
        syncActionBtn.onclick = function() {
            ...
            fetch('/SINTA-SaaS/api/v1/perpustakaan/anggota/sync')
            .then(r => r.json())
            .then(res => {
                if (res.success) {
                    alert(res.message);
    ...
    if (btnCekBebas) {
        btnCekBebas.onclick = function() {
            ...
            fetch('/SINTA-SaaS/api/v1/perpustakaan/bebas-pustaka/cek?siswa_id=' + encodeURIComponent(idSiswa))
            .then(r => r.json())
            .then(res => {
...
function loadVisitorLogs() {
    fetch('/SINTA-SaaS/api/v1/perpustakaan/visitor-logs')
    .then(r => r.json())
    .then(res => {
        if (res.success && res.data) {
