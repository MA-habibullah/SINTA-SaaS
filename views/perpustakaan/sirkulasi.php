...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-2 pb-2 mb-4 border-bottom">
    ...
    <div class="btn-toolbar gap-2 mb-2 mb-md-0">
        ...
        <a href="<?= $this->getBaseUrl() ?>/perpustakaan/kios-mandiri" target="_blank" class="btn btn-outline-success btn-sm rounded-3 px-3 py-2 fs-7">
...
function initSirkulasiHandlers() {
    ...
    if (formPinjam) {
        formPinjam.onsubmit = function(e) {
            ...
            fetch('<?= $this->getBaseUrl() ?>/api/v1/perpustakaan/sirkulasi/pinjam', {
                method: 'POST',
    ...
    if (formKembali) {
        formKembali.onsubmit = function(e) {
            ...
            fetch('<?= $this->getBaseUrl() ?>/api/v1/perpustakaan/sirkulasi/kembali', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
    ...
    payBtns.forEach(btn => {
        btn.onclick = function() {
            ...
            if (confirm('Apakah Anda yakin ingin memproses pembayaran denda tunai secara langsung?')) {
                fetch('<?= $this->getBaseUrl() ?>/api/v1/perpustakaan/denda/bayar', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
