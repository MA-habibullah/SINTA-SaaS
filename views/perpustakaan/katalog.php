        ...
        <div class="card border-0 shadow-sm rounded-4 p-4">
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <div class="d-flex gap-2">
                    <a href="<?= $this->getBaseUrl() ?>/perpustakaan/katalog/export-excel<?= !empty($data['active_tenant_id']) ? '?tenant_id=' . urlencode($data['active_tenant_id']) : '' ?>" class="btn btn-success btn-sm rounded-3 px-3 fs-7" title="Download Excel (.xlsx)">
                            ...
                            <?php foreach ($data['list'] as $i => $item): ?>
                                <tr>
                                    ...
                                    <td class="text-center">
                                        ...
                                        <a href="<?= $this->getBaseUrl() ?>/perpustakaan/cetak-label-thermal?barcode=<?= urlencode($item['isbn'] ?? 'BOOK-1') ?>" target="_blank" class="btn btn-outline-dark btn-sm rounded-2 me-1" title="Cetak Barcode Thermal">
                                            <i class="bi bi-printer"></i>
                                        </a>
                                        <?php if (!empty($item['is_ebook'])): ?>
                                            <a href="<?= $this->getBaseUrl() ?>/perpustakaan/baca-ebook" class="btn btn-outline-primary btn-sm rounded-2" title="Baca E-Book Watermark">
                                                <i class="bi bi-eye"></i>
                                            </a>
                                        <?php endif; ?>
                                    </td>
        ...
        <div class="card border-0 shadow-sm rounded-4 p-4">
            <div class="row">
                ...
                <div class="col-12 col-md-4 mb-4 mb-md-0 border-end">
                    ...
                    <form action="<?= $this->getBaseUrl() ?>/api/v1/perpustakaan/rak" method="POST" id="formSaveRak" data-turbo="false">
...
<div class="modal fade" id="modalTambahBuku" tabindex="-1" aria-labelledby="modalTambahBukuLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4">
            ...
            <form action="<?= $this->getBaseUrl() ?>/api/v1/perpustakaan/katalog/simpan" method="POST" id="formTambahBuku" data-turbo="false">
...
<div class="modal fade" id="modalTambahUsulan" tabindex="-1" aria-labelledby="modalTambahUsulanLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4">
            ...
            <form action="<?= $this->getBaseUrl() ?>/api/v1/perpustakaan/usulan" method="POST" id="formSaveUsulan" data-turbo="false">
...
<div class="modal fade" id="modalTambahSerial" tabindex="-1" aria-labelledby="modalTambahSerialLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4">
            ...
            <form action="<?= $this->getBaseUrl() ?>/api/v1/perpustakaan/serial" method="POST" id="formSaveSerial" data-turbo="false">
...
function initKatalogHandlers() {
    ...
    deleteUsulanBtns.forEach(btn => {
        btn.onclick = function() {
            ...
            if (confirm('Apakah Anda yakin ingin menghapus usulan pengadaan buku ini?')) {
                fetch('<?= $this->getBaseUrl() ?>/api/v1/perpustakaan/usulan?id=' + encodeURIComponent(id), {
                    method: 'DELETE'
                })
                .then(r => r.json())
                .then(res => {
    ...
    if (formSaveSerial) {
        formSaveSerial.onsubmit = function(e) {
            ...
            fetch('<?= $this->getBaseUrl() ?>/api/v1/perpustakaan/serial', {
                method: 'POST',
    ...
    deleteSerialBtns.forEach(btn => {
        btn.onclick = function() {
            ...
            if (confirm('Apakah Anda yakin ingin menghapus media berkala ini?')) {
                fetch('<?= $this->getBaseUrl() ?>/api/v1/perpustakaan/serial?id=' + encodeURIComponent(id), {
                    method: 'DELETE'
                })
                .then(r => r.json())
                .then(res => {
