...
<div id="dashboardApp">
    ...
    <div class="row mb-5" v-if="stats.pengumuman_list && stats.pengumuman_list.length > 0">
                        ...
                        <div class="card border border-light-subtle shadow-sm rounded-3">
                            <div class="card-body p-2 p-md-3">
                                ...
                                <div class="mt-3 pt-2 border-top" v-if="pengumuman.lampiran_file">
                                    <div class="mb-2 rounded-3 overflow-hidden shadow-sm" style="max-height: 150px; cursor: pointer; max-width: 250px;" v-if="isImageFile(pengumuman.lampiran_file)" @click="window.open('<?= $this->getBaseUrl() ?>/storage/app/public/' + pengumuman.lampiran_file, '_blank')">
                                        <img :src="'<?= $this->getBaseUrl() ?>/storage/app/public/' + pengumuman.lampiran_file" class="w-100 h-100 object-fit-cover hover-zoom" alt="Lampiran Pengumuman">
                                    </div>
                                    <a :href="'<?= $this->getBaseUrl() ?>/storage/app/public/' + pengumuman.lampiran_file" target="_blank" class="btn btn-primary fw-semibold d-flex d-md-inline-flex justify-content-center align-items-center rounded-3 px-3 py-1 shadow-sm w-100 w-md-auto" style="font-size: 0.8rem; transition: all 0.2s ease;">
        ...
        <div class="mt-2 mb-4 text-center" v-if="stats.total_pengumuman > stats.pengumuman_list.length">
            <a href="<?= $this->getBaseUrl() ?>/pengumuman/arsip" class="btn btn-outline-primary rounded-pill px-4 py-2 fw-semibold shadow-sm hover-shadow" style="transition: all 0.2s ease;">
        ...
        <div class="col-12">
            ...
            <div v-if="activeTab === 'siswa'" class="card border-0 shadow-sm rounded-4 p-3 p-md-4">
                <div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center gap-3 mb-4">
                    ...
                    <?php if ($stats['user_role'] !== 'siswa'): ?>
                        <a href="<?= $this->getBaseUrl() ?>/pengguna" class="btn btn-primary btn-sm rounded-3 px-3 py-2 shadow-sm fs-8 fs-sm-7">
        ...
        methods: {
            ...
            handleNpsnInput() {
                ...
                if (this.editForm.npsn.length === 8) {
                    ...
                    this.npsnError = '';
                    axios.get('<?= $this->getBaseUrl() ?>/api/v1/tenant/check-npsn', {
                        params: {
            ...
            saveProfil() {
                ...
                axios.post('<?= $this->getBaseUrl() ?>/api/v1/tenant/update', this.editForm)
                .then(response => {
                    this.isSaving = false;
            ...
            async refreshStats() {
                ...
                try {
                    const response = await axios.get('<?= $this->getBaseUrl() ?>/dashboard?ajax=1&action=get_dashboard_stats');
