...
<script>
{
    window.VueAppRegistry.register('#tenantManagementApp', {
        ...
        methods: {
            ...
            fetchTenants() {
                this.isLoading = true;
                axios.get('<?= $this->getBaseUrl() ?>/api/v1/super-admin/tenants')
                .then(response => {
            ...
            submitForm() {
                ...
                axios.post('<?= $this->getBaseUrl() ?>/api/v1/super-admin/tenants/simpan', this.form)
                .then(response => {
                    this.isSaving = false;
                    if (response.data.success) {
                        const m = this.getModal();
            ...
            saveStatus(id, newStatus, namaSekolah) {
                axios.post('<?= $this->getBaseUrl() ?>/api/v1/super-admin/tenants/toggle-status', {
                    id: id,
                    status: newStatus
                })
                .then(response => {
            ...
            deleteTenant(tenant) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        axios.post('<?= $this->getBaseUrl() ?>/api/v1/super-admin/tenants/hapus', {
                            id: tenant.id
                        })
                        .then(response => {
                            if (response.data.success) {
