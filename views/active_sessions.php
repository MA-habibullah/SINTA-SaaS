    ...
    window.VueAppRegistry.register('#activeSessionsApp', {
        setup() {
            ...
            const fetchData = async () => {
                loading.value = true;
                try {
                    const response = await axios.get('<?= $this->getBaseUrl() ?>/api/v1/sessions/data', {
                        params: {
            ...
            const fetchAuditLogs = async () => {
                ...
                try {
                    const response = await axios.get('<?= $this->getBaseUrl() ?>/api/v1/sessions/audit', {
                        params: {
            ...
            const executeRetention = () => {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        cleaning.value = true;
                        try {
                            const response = await axios.post('<?= $this->getBaseUrl() ?>/api/v1/sessions/retention', {
            ...
            const executeAuditRetention = () => {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        cleaning.value = true;
                        try {
                            const response = await axios.post('<?= $this->getBaseUrl() ?>/api/v1/sessions/audit/retention', {
