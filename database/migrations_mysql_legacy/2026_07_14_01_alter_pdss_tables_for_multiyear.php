...
return [
    ...
    'down' => function (PDO $pdo): void {
        // Rollback: Hapus kolom tahun_ajaran_id yang ditambahkan
        try {
            $pdo->exec("ALTER TABLE `pdss_config_mapel` DROP FOREIGN KEY `fk_pcm_tahun_ajaran`");
        } catch (\PDOException $e) {}
        try {
            $pdo->exec("ALTER TABLE `pdss_config_mapel` DROP INDEX `uk_tenant_ta_mapel`");
        } catch (\PDOException $e) {}
        try {
            $pdo->exec("ALTER TABLE `pdss_config_mapel` DROP COLUMN `tahun_ajaran_id`");
        } catch (\PDOException $e) {}
        try {
            $pdo->exec("ALTER TABLE `pdss_lock` DROP FOREIGN KEY `fk_pdss_lock_ta`");
        } catch (\PDOException $e) {}
        try {
            $pdo->exec("ALTER TABLE `pdss_lock` DROP COLUMN `tahun_ajaran_id`");
        } catch (\PDOException $e) {}
        echo "Rollback: Kolom multiyear PDSS dihapus.\n";
    },
];
