    ...
    public function fetchApi(): void {
        ...
        if ($isSuperAdmin && !empty($_GET['filter_tenant_id'])) {
            ...
            try {
                ...
                $stmtV = $db->prepare("SELECT id FROM tenants WHERE id = ? AND is_active = true LIMIT 1");
                $stmtV->execute([$tid]);
        ...
        if ($module === 'kelas') {
            ...
            if (!empty($namaKur)) {
                try {
                    ...
                    $sqlUq = "SELECT COUNT(*) FROM ref_kurikulum WHERE nama_kurikulum = ? AND is_active = true";
                    $uqParams = [$namaKur];
                    if ($tId) {
