...
class KategoriPengumumanModel {
    ...
    public function create(array $data): bool {
        $stmt = $this->db->prepare("INSERT INTO sistem.kategori_pengumuman (id, tenant_id, nama_kategori) VALUES (gen_random_uuid(), :tenant_id, :nama_kategori)");
        return $stmt->execute([
    ...
    public function update(string $id, string $nama_kategori): bool {
        $stmt = $this->db->prepare("UPDATE sistem.kategori_pengumuman SET nama_kategori = :nama_kategori WHERE id = :id");
        return $stmt->execute([
            'id' => $id,
