...
class ImportModuleController extends BaseController {
    ...
    public function __construct() {
        ...
        if ($roleName !== 'super_admin' && $roleName !== 'operator_sekolah') {
            $this->jsonResponse(false, null, 'Akses ditolak. Fitur ini memerlukan wewenang Admin Sekolah atau Super Admin.', 403);
        }
    }
    ...
    public function import(): void {
        ...
        if (!$xlsx) {
            $this->jsonResponse(false, null, 'Gagal membaca berkas Excel (.xlsx): ' . \Shuchkin\SimpleXLSX::parseError(), 400);
        }

        $rows = $xlsx->rows();
        if (empty($rows)) {
            $this->jsonResponse(false, null, 'Berkas Excel kosong atau tidak valid.', 400);
        }
        ...
        if ($namaIdx === -1 || $nisnIdx === -1 || $tglLahirIdx === -1 || $emailIdx === -1) {
            $this->jsonResponse(false, null, 'Kolom header file Excel tidak sesuai dengan format template (Kolom wajib: Nama Lengkap Siswa, NISN, Tanggal Lahir, Email).', 400);
        }
        ...
        if ($roleName === 'super_admin' && $npsnIdx === -1) {
            $this->jsonResponse(false, null, 'Untuk peran Super Admin, kolom NPSN Sekolah wajib dicantumkan di dalam file Excel.', 400);
        }
        ...
        try {
            $stmtTenantLookup = $db->prepare("SELECT id FROM core.tenants WHERE npsn = :npsn AND deleted_at IS NULL LIMIT 1");
            $stmtNisnCheck = $db->prepare("SELECT COUNT(*) FROM siswa.siswa WHERE nisn = :nisn AND deleted_at IS NULL");
            $stmtSiswaInsert = $db->prepare("
                INSERT INTO siswa.siswa (
            ...
            $stmtAngkatanLookup = $db->prepare("SELECT id FROM akademik.angkatan WHERE tenant_id = ? AND tahun_angkatan = ? LIMIT 1");
            $stmtAngkatanInsert = $db->prepare("INSERT INTO akademik.angkatan (tenant_id, tahun_angkatan, is_active) VALUES (?, ?, true)");
            $stmtTaLookup = $db->prepare("SELECT id FROM akademik.tahun_ajaran WHERE tenant_id = ? AND tahun_ajaran = ? LIMIT 1");
            $stmtTaInsert = $db->prepare("INSERT INTO akademik.tahun_ajaran (tenant_id, tahun_ajaran, is_active) VALUES (?, ?, true)");
            ...
            if (!empty($errors)) {
                $db->rollBack();
                $this->jsonResponse(false, ['errors' => $errors], 'Gagal memproses file. Beberapa baris data tidak valid.', 422);
            }
            ...
            $this->jsonResponse(false, null, 'Terjadi kesalahan sistem saat menyimpan data: ' . $e->getMessage(), 500);
        }
    }
