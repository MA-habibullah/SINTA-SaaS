...
const activeTab = ref('aktif') // 'aktif' | 'riwayat' | 'reservasi' | 'kta_digital'
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
          <div class="flex items-center gap-4">
            ...
            <div>
              ...
              <p class="text-xs text-slate-300 mt-0.5">Portal Layanan Mandiri Pemustaka Perpustakaan</p>
            </div>
          </div>
          ...
          <div class="flex items-center gap-3">
            <div v-if="isBebasPustaka" class="p-3 rounded-2xl bg-emerald-500/20 border border-emerald-400/30 backdrop-blur-md flex items-center gap-3">
              ...
              <div>
                ...
                <div class="text-[11px] text-emerald-100">Clear • Tidak ada tanggungan pinjaman/denda</div>
              </div>
            </div>
            <div v-else class="p-3 rounded-2xl bg-rose-500/20 border border-rose-400/30 backdrop-blur-md flex items-center gap-3">
              ...
              <div>
                ...
                <div class="text-[11px] text-rose-100">
                  {{ pinjamanAktif?.length || 0 }} Buku Dipinjam • Denda: Rp {{ Number(totalDenda || 0).toLocaleString('id-ID') }}
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
        <div class="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-2xs">
          ...
          <div class="text-xl font-black text-blue-600 mt-2">{{ pinjamanAktif?.length || 0 }} Buku</div>
        ...
        <div class="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-2xs">
          ...
          <div class="text-xl font-black text-purple-700 mt-2">{{ riwayatSelesai?.total || riwayatSelesai?.data?.length || 0 }} Kali</div>
        ...
        <div class="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-2xs">
          ...
          <div class="text-xl font-black text-emerald-600 mt-2">{{ reservasiSaya?.length || 0 }} Judul</div>
      ...
      <div v-if="activeTab === 'aktif'" class="space-y-4">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div v-for="item in (pinjamanAktif || [])" :key="item.id" class="bg-white rounded-3xl border border-slate-200/80 shadow-2xs p-5 flex flex-col justify-between hover:border-blue-300 transition">
            <div>
        ...
        <div v-if="!pinjamanAktif?.length" class="text-center py-12 bg-white rounded-3xl border border-slate-200/80 p-8">
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse text-xs">
              ...
              <tbody class="divide-y divide-slate-100">
                <tr v-for="res in (reservasiSaya || [])" :key="res.id" class="hover:bg-blue-50/30 transition">
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-2xs overflow-hidden">
          <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse text-xs">
              ...
              <tbody class="divide-y divide-slate-100">
                <tr v-for="item in (riwayatSelesai?.data || [])" :key="item.id" class="hover:bg-blue-50/30 transition">
                ...
                <tr v-if="!riwayatSelesai?.data?.length">
        ...
        <div class="max-w-md mx-auto border-2 border-slate-800 rounded-3xl p-6 bg-gradient-to-r from-blue-900 via-indigo-900 to-slate-900 text-white shadow-2xl space-y-4 relative overflow-hidden">
          ...
          <div class="flex items-center justify-between border-b border-white/20 pb-3">
            <div>
              ...
              <p class="text-[9px] text-slate-300">SINTA Digital Library Card</p>
            </div>
