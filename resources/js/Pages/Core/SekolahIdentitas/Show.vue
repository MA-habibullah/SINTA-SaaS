...
const localIdentitas = ref(props.identitas || null)
const localTenantsList = ref(props.tenantsList || [])
const isLoading = ref(!props.identitas)
const selectedTenantId = ref(props.identitas?.id || '')
...
useMemorySecurity([form, localIdentitas, localTenantsList])
...
const tenantOptions = computed(() => {
  return (localTenantsList.value || []).map(t => ({
    id: t.id,
    nama: t.nama_sekolah,
...
const bentukPendidikanOptions = [
  ...
  { id: 'MI', nama: 'MI (Madrasah Ibtidaiyah)' },
  { id: 'MTs', nama: 'MTs (Madrasah Tsanawiyah)' },
  { id: 'MA', nama: 'MA (Madrasah Aliyah)' },
  { id: 'SLB', nama: 'SLB (Sekolah Luar Biasa)' },
  { id: 'Lainnya', nama: 'Lainnya' },
]

const statusSekolahOptions = [
  { id: 'Negeri', nama: 'Negeri' },
  { id: 'Swasta', nama: 'Swasta' },
]

// Populate form helper
const populateForm = (data) => {
  if (!data) return
  form.tenant_id = data.id || ''
  form.nama_sekolah = data.nama_sekolah || ''
  form.npsn = data.npsn || ''
  form.bentuk_pendidikan = data.bentuk_pendidikan || ''
  form.status_sekolah = data.status_sekolah || ''
  form.kurikulum_terapan = data.kurikulum_terapan || ''
  form.akreditasi = data.akreditasi || ''
  form.subdomain = data.subdomain || ''
  form.alamat = data.alamat || ''
  form.rt_rw = data.rt_rw || ''
  form.kode_pos = data.kode_pos || ''
  form.kelurahan = data.kelurahan || ''
  form.kecamatan = data.kecamatan || ''
  form.kabupaten_kota = data.kabupaten_kota || ''
  form.provinsi = data.provinsi || ''
  form.telepon = data.telepon || ''
  form.email = data.email || ''
  form.website = data.website || ''
  form.nama_kepsek = data.nama_kepsek || ''
  form.pangkat_kepsek = data.pangkat_kepsek || ''
  form.nip_kepsek = data.nip_kepsek || ''
  form.nama_operator = data.nama_operator || ''
  form.email_operator = data.email_operator || ''

  logoFile.value = null
  deleteLogoFlag.value = false
  logoPreview.value = data.logo 
    ? (data.logo.startsWith('http') ? data.logo : `/storage/${data.logo}`) 
    : null

  certFile.value = null
  certFileName.value = ''
  certExisting.value = data.sertifikat_akreditasi || null
  deleteCertFlag.value = false
}

// On-Demand Client Fetch (Zero-SSR Data Hydration)
const loadIdentitasAsync = async (tenantId = null) => {
  isLoading.value = true
  errors.value = {}
  try {
    const params = { async: 1 }
    if (tenantId) {
      params.tenant_id = tenantId
    }
    const res = await axios.get('/sekolah/identitas', { params })
    if (res.data && res.data.success) {
      localIdentitas.value = res.data.data
      if (res.data.tenantsList && res.data.tenantsList.length > 0) {
        localTenantsList.value = res.data.tenantsList
      }
      populateForm(res.data.data)
      selectedTenantId.value = res.data.data?.id || ''
    }
  } catch (err) {
    console.error('Gagal memuat profil sekolah:', err)
    triggerToast('Gagal memuat profil sekolah dari server.', 'error')
  } finally {
    isLoading.value = false
  }
}

// In-Memory Tenant Switcher for Super Admin (No URL Pollution)
const handleTenantChange = (newTenantId) => {
  if (newTenantId) {
    selectedTenantId.value = newTenantId
    loadIdentitasAsync(newTenantId)
  }
}

// Watch prop if passed directly
watch(() => props.identitas, (newVal) => {
  if (newVal) {
    localIdentitas.value = newVal
    populateForm(newVal)
  }
}, { deep: true })

onMounted(() => {
  if (!props.identitas || !props.identitas.id) {
    loadIdentitasAsync()
  } else {
    populateForm(props.identitas)
    if (props.tenantsList) {
      localTenantsList.value = props.tenantsList
    }
    selectedTenantId.value = props.identitas.id
    isLoading.value = false
  }
})
...
const processLogoFile = (file) => {
  if (file.size > 2 * 1024 * 1024) {
    triggerToast('Ukuran file logo maksimal 2 MB!', 'error')
    return
  }
  const validTypes = ['image/jpeg', 'image/png', 'image/jpg', 'image/webp', 'image/svg+xml']
  if (!validTypes.includes(file.type)) {
    triggerToast('Format file logo harus berupa gambar (JPG, PNG, WebP, SVG)!', 'error')
    return
  }
  logoFile.value = file
  deleteLogoFlag.value = false
...
const processCertFile = (file) => {
  if (file.size > 5 * 1024 * 1024) {
    triggerToast('Ukuran file sertifikat maksimal 5 MB!', 'error')
    return
  }
  certFile.value = file
...
const submitForm = async () => {
  ...
  // Explicit target tenant if super admin
  if (isSuperAdminComputed.value && selectedTenantId.value) {
    formData.set('tenant_id', selectedTenantId.value)
  }
  ...
  try {
    const res = await axios.post('/sekolah/identitas', formData, {
      headers: {
    ...
    if (res.data && res.data.success) {
      localIdentitas.value = res.data.data
      populateForm(res.data.data)
      triggerToast(res.data.message || 'Identitas dan profil sekolah berhasil disimpan!', 'success')
    } else {
      triggerToast('Identitas berhasil diperbarui.', 'success')
    }
  } catch (err) {
    ...
    if (err.response && err.response.status === 422) {
      ...
      triggerToast(err.response?.data?.message || 'Terjadi kesalahan sistem saat menyimpan profil.', 'error')
    }
  } finally {
...
<template>
  <AppLayout title="Identitas Sekolah">
    <div class="max-w-7xl mx-auto space-y-6 pb-12">
      ...
      <!-- Super Admin Filter Bar (SearchableSelect In-Memory Switcher) -->
      <div v-if="isSuperAdminComputed && localTenantsList && localTenantsList.length > 0" class="bg-white rounded-3xl p-5 shadow-2xs border border-slate-200/80 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      ...
      <div v-if="isLoading" class="space-y-6 animate-pulse">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div class="space-y-6">
            ...
            <div class="h-48 bg-slate-200 rounded-3xl"></div>
          </div>
          <div class="lg:col-span-2 space-y-6">
            <div class="h-80 bg-slate-200 rounded-3xl"></div>
            <div class="h-80 bg-slate-200 rounded-3xl"></div>
          </div>
        </div>
      </div>
            ...
            <div class="flex items-center gap-5">
              ...
              <div class="space-y-1">
                <div class="flex flex-wrap items-center gap-2">
                  <h1 class="text-xl sm:text-2xl font-black tracking-tight text-white">{{ form.nama_sekolah || localIdentitas?.nama_sekolah || 'Nama Sekolah Belum Diisi' }}</h1>
                  <span class="bg-emerald-500/90 text-white text-[11px] font-bold px-2.5 py-0.5 rounded-full shadow-xs border border-emerald-400/30 flex items-center gap-1.5">
                    ...
                    {{ localIdentitas?.status === 'active' || localIdentitas?.status === 'aktif' ? 'Tenant Aktif' : 'Aktif' }}
                  </span>
                </div>
          ...
          <div class="space-y-6">
            ...
            <div class="bg-white rounded-3xl p-6 shadow-2xs border border-slate-200/80 flex flex-col items-center text-center relative">
              ...
              <button 
                ...
                type="button" 
                @click="removeLogo"
                class="mt-3 text-xs font-bold text-red-500 hover:text-red-700 px-3 py-1 rounded-xl hover:bg-red-50 transition flex items-center gap-1.5 cursor-pointer"
              >
              ...
              <div class="w-full bg-slate-50 rounded-2xl p-4 text-left border border-slate-100 flex flex-col gap-2.5 mt-5">
                ...
                <div class="flex justify-between items-center text-xs">
                  ...
                  <span class="text-blue-600 font-bold font-mono">{{ localIdentitas?.paket_aktif || 'Enterprise' }}</span>
                </div>
                <div class="flex justify-between items-center text-xs border-t border-slate-200/80 pt-2.5">
                  ...
                  <span class="font-bold text-emerald-600 flex items-center gap-1">
                    <i class="bi bi-check-circle-fill"></i> {{ localIdentitas?.status_sinkronisasi || 'Tersinkronisasi' }}
                  </span>
                </div>
              </div>
            </div>
              ...
              <div class="space-y-2">
                ...
                <div v-if="certExisting && !certFile" class="flex items-center justify-between p-3.5 bg-slate-50 rounded-2xl border border-slate-200">
                  ...
                  <div class="flex items-center gap-2 shrink-0">
                    ...
                    <button type="button" @click="removeCert" class="p-1.5 text-red-500 hover:text-red-700 hover:bg-red-50 rounded-xl transition cursor-pointer" title="Hapus Berkas">
                ...
                <div v-else-if="certFile" class="flex items-center justify-between p-3.5 bg-emerald-50 rounded-2xl border border-emerald-200">
                  ...
                  <button type="button" @click="removeCert" class="p-1.5 text-red-500 hover:text-red-700 hover:bg-red-50 rounded-xl transition cursor-pointer" title="Batalkan">
          ...
          <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-3xl p-6 sm:p-8 shadow-2xs border border-slate-200/80 space-y-6">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <!-- Bentuk Pendidikan (SearchableSelect) -->
                <div class="space-y-1.5">
                  ...
                  <SearchableSelect
                    ...
                    custom-class="w-full"
                  />
                  <span v-if="errors.bentuk_pendidikan" class="text-xs text-red-500 font-medium">{{ errors.bentuk_pendidikan }}</span>
                </div>

                <!-- Status Sekolah (SearchableSelect) -->
                <div class="space-y-1.5">
                  ...
                  <SearchableSelect
                    ...
                    custom-class="w-full"
                  />
                  <span v-if="errors.status_sekolah" class="text-xs text-red-500 font-medium">{{ errors.status_sekolah }}</span>
                </div>
